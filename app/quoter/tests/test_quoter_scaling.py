...
import pytest
from app.quoter.quoter import LiquoriceQuoter, QUOTE_PREMIUM
...
def quoter(mock_market_state):
    ...
    out_q = asyncio.Queue()
    signer = Mock()
    signer.sign_quote_levels = Mock(side_effect=lambda rfq, quote: quote) # Return unsigned quote for inspection
...
async def test_quote_scaling_insufficient_liquidity(quoter, mock_market_state):
    ...
    mock_market_state.get_token.side_effect = lambda addr, chain: base_token if addr == base_token_addr else quote_token
